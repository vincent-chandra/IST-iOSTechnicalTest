...
class ViewController: UIViewController, AnyView {
    ...
    override func viewDidLoad() {
        ...
        MovieInteractor.shared.AFMovieList(page: currentPage) { movie in
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if (indexPath.row == movieData.results!.count - 5){
            ...
            if movieSearchBar.searchTextField.text!.isEmpty{
                ...
                MovieInteractor.shared.AFMovieSearch(page: currentPage, title: movieSearchBar.text!, completion: { movie in
                    DispatchQueue.main.async {
                        self.movieData.results?.append(contentsOf: movie.results!)
                        self.movieCollectionView.reloadData()
                    }
...
extension ViewController: UISearchBarDelegate{
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if !searchText.isEmpty{
            ...
            MovieInteractor.shared.AFMovieSearch(page: currentPage, title: searchedText) { movie in
                DispatchQueue.main.async {
                    self.movieData = movie
                    self.movieCollectionView.reloadData()
                }
